...
export default function ApartmentsManager() {
  ...
  const handleEdit = (apartment) => {
    ...
    setFormData({
      ...
      images: Array.isArray(apartment.images) ? apartment.images.join(', ') : '',
      ...
      {showForm && (
        <Modal onClose={() => { setShowForm(false); setEditingApartment(null); }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Или загрузить фото с компьютера</label>
              <input
                type="file"
                multiple
                accept="image/*"
