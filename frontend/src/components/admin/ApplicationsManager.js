              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusConfig[a.status]?.color || "bg-gray-100 text-gray-800"}`}
                    >
                    ...
                    <div className="flex space-x-2">
                      ...
                      {a.status === "new" && (
                        <ActionButton
                          ...
                          Icon={MessageCircle}
                          onClick={() =>
                            updateStatus(a.id || a._id, "in_progress")
                          }
                        />
                      )}
                      {a.status === "in_progress" && (
                        <ActionButton
                          ...
                          Icon={CheckCircle}
                          onClick={() =>
                            updateStatus(a.id || a._id, "completed")
                          }
                        />
                      )}
                      <ActionButton
                        ...
                        color="red"
                        Icon={X}
                        onClick={() => deleteApplication(a.id || a._id)}
                      />
                    </div>
                  </td>
                </tr>
...
function ModalContent({ app, onChangeStatus, onDelete, onClose }) {
  const statusActions = {
  ...
  return (
    <div className="space-y-4">
      ...
      <InfoRow
        label="Дата"
        value={new Date(app.timestamp || app.date).toLocaleString()}
      />
      <InfoRow
        label="Проект"
        value={`${app.projectName}${app.apartmentNumber ? ` • №${app.apartmentNumber}` : ""}`}
      />
      ...
      <div className="flex space-x-3 pt-4">
        {action && (
          <button
            className={`bg-${action.color}-500 hover:bg-${action.color}-600 text-white px-4 py-2 rounded`}
            onClick={async () => {
              await onChangeStatus(app.id || app._id, action.next);
              onClose();
            }}
          >
            {action.label}
...
const InfoRow = ({ label, value, pre }) => (
  <div>
    ...
    {pre ? (
      <pre className="bg-gray-50 p-3 rounded whitespace-pre-wrap break-words">
        {value}
      </pre>
    ) : (
