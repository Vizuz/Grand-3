    ...
    <div className="space-y-6 p-6">
      ...
      {selectedApp && (
        <Modal onClose={() => setSelectedApp(null)}>
          <ModalContent app={selectedApp} onChangeStatus={updateStatus} onDelete={deleteApplication} onClose={() => setSelectedApp(null)} />
        </Modal>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex space-x-3 pt-4">
        {action && <button className={`bg-${action.color}-500 hover:bg-${action.color}-600 text-white px-4 py-2 rounded`} onClick={async () => { await onChangeStatus(app.id || app._id, action.next); onClose(); }}>{action.label}</button>}
        <button className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded" onClick={async () => { await onDelete(app.id || app._id); onClose(); }}>Удалить</button>
      </div>
    </div>
  );
}
